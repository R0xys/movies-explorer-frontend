    ...
    <div className={`${props.isBurgerMenuOpen ? 'burger-menu_open' : 'burger-menu'}`}>
      <div className={`${props.isBurgerMenuOpen ? 'burger-menu__container_open' : 'burger-menu__container'}`}>
        ...
        <nav className='burger-menu__links'>
          ...
          <NavLink onClick={handleClose} to='/profile' className={({ isActive }) => isActive ? 'burger-menu__link_active burger-menu__link_type_account zero-link' : 'burger-menu__link_type_account zero-link'}>Аккаунт</NavLink>
        </nav>
      </div>
    </div>
  )
